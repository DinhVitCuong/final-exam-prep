...
def select_uni():
    ...
    if form.validate_on_submit():
        ...
        budget = int(form.budget.data) if form.budget.data.isdigit() else 0
                ...
                for uni in universities:
                    try:
                        ...
                        if len(budget_range) == 2:
                            ...
                            upper_budget = int(budget_range[1].replace('tr', '').strip())

                            if budget >= lower_budget and budget <= upper_budget:
        ...
        if current_slide>=5:
            selected_university = Universities.query.filter_by(name=form.university.data).first()
        if current_slide==6:
            ...
            print("check clicked")
            return render_template("select_uni.html", form=form, score= majors_score, current_slide=current_slide+1)
